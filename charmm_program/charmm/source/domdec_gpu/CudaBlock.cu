...
// Class creator
//
CudaBlock::CudaBlock(const int numBlock,const int use_softcore,const int use_PMEL) : numBlock(numBlock) {
  assert(numBlock >= 1);
  blockTypeLen = 0;
  blockType = NULL;
...
void CudaBlock::setBlockParam(const float *h_blockParamFull) {
  ...
  if (usePMEL>=2) {
    copy_HtoD_sync<float>(h_blockParamEx, d_blockParamEx, numBlock*(numBlock+1)/2);
  }
}
