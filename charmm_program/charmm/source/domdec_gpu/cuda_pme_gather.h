...
#pragma unroll
  ...
  // while (base < base_end)
  while (base < blockDim.x) { // need to go past base_end to avoid problems with shuffle reductions
    ...
    float f2 = 0.0f;
    float f3 = 0.0f;
    if (base < base_end) { // but don't read data past base_end or there will be seg faults
...
#pragma unroll
    ...
    // Reduce
#if __CUDA_ARCH__ >= 300
    } // end if(base<base_end) before reduction if using shuffle
    ...
    } // end if(base<base_end) after reduction if not using shuffle
#endif
