...
#ifdef USE_BLOCK
                                     const int* in14TblBlockPos,
                                     const int* ex14TblBlockPos,
                                     const int* in14BlockToBlock,
                                     const int* ex14BlockToBlock,
                                     const int* in14BlockToTblPos,
                                     const int* ex14BlockToTblPos,
#endif
...
// #ifdef USE_BLOCK_SOFTCORE
  extern __shared__ double3 shpot[]; // Only need double3 if soft cores are in use, but compiler doesn't seem smart enough to see that, or else I've missed a subtle bug
// #else
...
// #endif
  ...
  if (blockIdx.x < nin14block) { // for USE_BLOCK, this is ALL nin14block
...
#ifdef USE_BLOCK
#ifdef USE_TEXTURE_OBJECTS
    float fscale = tex1Dfetch<float>(blockParamTexObj,
      in14BlockToBlock[blockIdx.x]);
#else
    float fscale = tex1Dfetch(blockParamTexRef,
      in14BlockToBlock[blockIdx.x]);
#endif
    ...
    {
#ifdef USE_BLOCK_SOFTCORE
      calc_in14_force_device<AT, CT, vdw_model, elec_model, calc_energy, calc_virial, true, tex_vdwparam>
#else
      calc_in14_force_device<AT, CT, vdw_model, elec_model, calc_energy, calc_virial, false, tex_vdwparam>
#endif
	(
#ifdef USE_TEXTURE_OBJECTS
	 vdwParamTexObj, vdwParam14TexObj, 
#endif
	 pos, in14list, vdwtype, vdwparam, vdwparam14, xyzq, fscale, stride, force, vdw_pot, elec_pot,
         soft_f, virial);
    }
  ...
  } else if (elec_model == EWALD || elec_model == EWALD_LOOKUP || elec_model == ELE_IPS) {
    // NOTE: Only Ewald and IPS potentials calculate 1-4 exclusions
    double excl_pot,excl_vdw;
    if (calc_energy) {
        excl_vdw = 0.0;
        excl_pot = 0.0;
    }
...
#endif
    {
      calc_ex14_force_device<AT, CT, vdw_model, elec_model, calc_energy, calc_virial, tex_vdwparam>
	(
#ifdef USE_TEXTURE_OBJECTS
	 vdwParamTexObj,
#endif
            pos, ex14list, vdwtype, vdwparam, xyzq, fscale, stride, force, excl_vdw, excl_pot, virial);
    }

    if (calc_energy) {
      shpot[threadIdx.x].x = excl_vdw;
      shpot[threadIdx.x].y = excl_pot;
      __syncthreads();
      for (int i=1;i < blockDim.x;i *= 2) {
	int t = threadIdx.x + i;
	double val1 = (t < blockDim.x) ? shpot[t].x : 0.0;
	...
	__syncthreads();
      ...
      if (threadIdx.x == 0) {
          if(elec_model == ELE_IPS){
#ifdef USE_BLOCK
            atomicAdd(&energy_vdw[ex14BlockToBlock[blockIdx.x-nin14block]],  shpot[0].x);
            atomicAdd(&energy_elec[ex14BlockToBlock[blockIdx.x-nin14block]], shpot[0].y);
#else
            atomicAdd(energy_vdw,  shpot[0].x);
            atomicAdd(energy_elec, shpot[0].y);
#endif              
...
#ifdef USE_BLOCK
        atomicAdd(&energy_excl[ex14BlockToBlock[blockIdx.x-nin14block]],  shpot[0].y);
#else
        atomicAdd(energy_excl,  shpot[0].y);
#endif
          }
      }
    }
