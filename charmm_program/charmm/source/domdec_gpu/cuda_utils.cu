...
int selectGpu(int localRank, int wantedGpu) {
  int
    selectedGpu = -1,
    nDevices = 0;
  ...
  if (localRank >= 0 && wantedGpu < 0)  // any gpu will do
    selectedGpu = localRank;
  else if (wantedGpu >= 0)  // want a specific gpu
    selectedGpu = wantedGpu;
  else  // no local rank and no gpu specified
    selectedGpu = 0;

  selectedGpu = selectedGpu % nDevices;
  
  std::vector<std::pair<int, cudaDeviceProp> > devices(nDevices);
  for (int i = 0; i < nDevices; ++i) {
    devices.at(i).first = i;
    cudaCheck(cudaGetDeviceProperties(&devices.at(i).second, i));
  }

  // TODO: wantedGpu == -1 => sort by cuda number then proc count
  //       wantedGpu == -2 => sort by whether device is busy  
  if (wantedGpu < 0)
    std::sort(devices.begin(), devices.end(), gpuDevCompare);

  selectedGpu = devices.at(selectedGpu).first;
  cudaCheck(cudaSetDevice(selectedGpu));
  return selectedGpu;
}

int start_gpu(int prnlev, int localRank, int globalRank, int wantedGpu) {
  gpu_ind = selectGpu(localRank, wantedGpu);
  cudaCheck(cudaDeviceSynchronize());
  ...
  if (cuda_arch < 300 || cuda_rt_version < 5000 || cuda_driver_version < 5000) {
    std::cout << "When compiled with USE_TEXTURE_OBJECTS, CUDA 5.0 "
              << "and compute capability 3.0 (Kepler) or greater required"
              << std::endl;
    exit(1);
  }
#endif
  ...
  if (globalRank == 0) {
    ...
    std::cout << "Using CUDA driver version " << cuda_driver_version
              << std::endl;
    std::cout << "Using CUDA runtime version " << cuda_rt_version << std::endl;
  }

  if (prnlev >= 2) {
    std::cout << "Node " << globalRank << " uses CUDA device " << gpu_ind
              << " " << gpu_prop.name << " with CUDA_ARCH " << cuda_arch
              << std::endl;
  }
  return gpu_ind;
}
