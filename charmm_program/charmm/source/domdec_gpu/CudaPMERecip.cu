...
//
template <typename AT, typename CT, typename CT2>
void CudaPMERecip<AT, CT, CT2>::spread_charge_block(const float4 *xyzq, const int ncoord,
// MSLDPME Addition ->
...
// <- MSLDPME Addition
    ...
    break;
  default:
    std::cerr << "CudaPMERecip::spread_charge: order "
              << order
              << " not implemented"
              << std::endl;
    exit(1);
...
//
template <typename AT, typename CT, typename CT2>
void CudaPMERecip<AT, CT, CT2>::calc_self_energy_block(const float4 *xyzq, const int ncoord, const double kappa
// MSLDPME Addition ->
...
void CudaPMERecip<AT, CT, CT2>::r2c_fft() {
  ...
  if (fft_type == COLUMN) {
	...
	for (int y=0;y < ysize;y++)
      ...
      cudaCheck(cudaMemcpy(fft_grid->data,
                           fft_scratch,
...
template <typename AT, typename CT, typename CT2>
template <typename FT>
void CudaPMERecip<AT, CT, CT2>::gather_force_block(const float4 *xyzq, const int ncoord,
                                                   const double* recip,
                                                   const int stride,
                                                   FT * force,
                                                   const float * bixlam,
  ...
  if (ortho) {
    ...
    case 4:
      gather_force_4_ortho_block_kernel<CT, FT> 
	<<< nblock, nthread, 0, stream >>>
	(xyzq, ncoord,
	 nfftx, nffty, nfftz,
	 nfftx, nffty, nfftz,
    ...
    case 6:
      gather_force_6_ortho_block_kernel<CT, FT> 
	<<< nblock, nthread, 0, stream >>>
	(xyzq, ncoord,
	 nfftx, nffty, nfftz,
	 nfftx, nffty, nfftz,
    ...
    case 8:
      gather_force_8_ortho_block_kernel<CT, FT> 
	<<< nblock, nthread, 0, stream >>>
	(xyzq, ncoord,
	 nfftx, nffty, nfftz,
	 nfftx, nffty, nfftz,
    ...
    default:
      std::cerr << "CudaPMERecip::gather_force: order "
                << order
                << " not implemented"
                << std::endl;
      exit(1);
      ...
      std::cerr << "CudaPMERecip::gather_force: only orthorombic boxes are currently supported"
                << std::endl
                << recip[1] << std::endl
                << recip[2] << std::endl
                << recip[3] << std::endl
...
template void CudaPMERecip<int, float, float2>::gather_force_block<float>(const float4 *xyzq, const int ncoord,
                                                                          const double* recip,
                                                                          const int stride,
                                                                          float* force,
                                                                          const float * bixlam,
                                                                          double * biflam,
                                                                          const int * blockIndexes);
template void CudaPMERecip<int, float, float2>::gather_force_block<long long int>(const float4 *xyzq, const int ncoord,
                                                                                  const double* recip,
                                                                                  const int stride, long long int* force,
                                                                                  const float * bixlam, double * biflam, const int * blockIndexes);
template void CudaPMERecip<int, float, float2>::gather_force_block<float3>(const float4 *xyzq, const int ncoord,
                                                                           const double* recip,
