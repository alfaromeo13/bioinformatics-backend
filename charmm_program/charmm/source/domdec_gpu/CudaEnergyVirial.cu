...
//
__global__ void calcVirialKernel(const int ncoord, const float4* __restrict__ xyzq,
				 const double boxx, const double boxy, const double boxz,
				 const int stride, const double* __restrict__ force,
...
//
CudaEnergyVirial::~CudaEnergyVirial() {
  if (d_buffer != NULL) deallocate<char>(&d_buffer);
  if (h_buffer != NULL) deallocate_host<char>(&h_buffer);
}
...
void CudaEnergyVirial::reallocateBuffer() {
  ...
  int buffer_len_req = 27*3*sizeof(double) + 27*3*sizeof(long long int) + 9*sizeof(double) + this->getN()*sizeof(double);
...
//
void CudaEnergyVirial::clearEtermDevice(std::string& nameEterm, cudaStream_t stream) {
  this->reallocateBuffer();
  int clear_pos = 27*3*sizeof(double) + 27*3*sizeof(long long int) + 9*sizeof(double) + this->getEnergyIndex(nameEterm)*sizeof(double);
...
void CudaEnergyVirial::calcVirial(const int ncoord, const float4 *xyzq,
  ...
  calcVirialKernel<<< nblock, nthread, shmem_size, stream >>>
    (ncoord, xyzq, boxx, boxy, boxz, stride, force,
     this->getVirialPointer());
...
double* CudaEnergyVirial::getEnergyPointer(std::string& name) {
  this->reallocateBuffer();
  return (double *)(&d_buffer[27*3*sizeof(double) + 27*3*sizeof(long long int) + 9*sizeof(double) + this->getEnergyIndex(name)*sizeof(double)]);
}
...
double CudaEnergyVirial::getEnergy(std::string& name) {
  this->reallocateBuffer();
  double *p = (double *)(&h_buffer[27*3*sizeof(double) + 27*3*sizeof(long long int) + 9*sizeof(double) + this->getEnergyIndex(name)*sizeof(double)]);
  return *p;
}
...
void CudaEnergyVirial::getVirial(double *virmat) {
  this->reallocateBuffer();
  double *p = (double *)(&h_buffer[27*3*sizeof(double) + 27*3*sizeof(long long int)]);
  for (int i=0;i < 9;i++) virmat[i] = p[i];
}
